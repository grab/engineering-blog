...
After several misfiring events (wrong implementation of rules) and ever longer rule development time (weekly), we realized “_No one can handle this manually_.“
...
Based on these findings, we got started with our Data Orchestrator (open sourced at [https://github.com/grab/symphony](https://github.com/grab/symphony&sa=D&ust=1572350981981000)) and Griffin projects.
...
Griffin  now predicts billions of events every day with 100K+ Queries per second(QPS) at peak time (on only 6 regular [EC2s](https://aws.amazon.com/ec2/&sa=D&ust=1572350981983000)).
...
Before we decided to create our in-built tool, we did some research for common [business rule engines](https://en.wikipedia.org/wiki/Business_rules_engine&sa=D&ust=1572350981984000) available in the market such as [Drools](https://en.wikipedia.org/wiki/Drools&sa=D&ust=1572350981985000) and checked if we should use them. In that process, we found:

1.  Drools has its own Java-based [DSL](https://en.wikipedia.org/wiki/Domain-specific_language&sa=D&ust=1572350981985000) with a non-trivial learning curve (whereas our major users are from Python background).
...
*   So the whole prediction flow is only between CPU & memory (and if the data size is small, it could be on [CPU cache](https://www.prowesscorp.com/computer-latency-at-a-human-scale/&sa=D&ust=1572350981999000) only).
*   [Python GIL](https://wiki.python.org/moin/GlobalInterpreterLock&sa=D&ust=1572350981999000) essentially enforces a process to have up to one active thread running at a time, no matter how many cores a CPU has. We have [Gunicorn](https://gunicorn.org/&sa=D&ust=1572350982000000) to wrap our service, so on the Production machine, we have `(2x$num_cores) + 1 processes` (see [http://docs.gunicorn.org/en/latest/design.html#how-many-workers](http://docs.gunicorn.org/en/latest/design.html%23how-many-workers&sa=D&ust=1572350982000000)). The formula is based on the assumption that for a given core, one worker will be reading or writing from the socket while the other worker is processing a request.

The below screenshot is the process snapshot on [C5.large machine](https://aws.amazon.com/ec2/instance-types/c5/&sa=D&ust=1572350982001000) with 2 vCPU. Note only green processes are active.
...
*   We used to have [python-jsonpath-rw](https://github.com/kennknowles/python-jsonpath-rw&sa=D&ust=1572350982002000) for JSONPath query, but the performance was not strong enough. We switched to [jmespath](https://github.com/jmespath/jmespath.py&sa=D&ust=1572350982003000) and observed about 10ms latency reduction.
*   We use [sqlalchemy](https://www.sqlalchemy.org&sa=D&ust=1572350982003000) for DB Query and ORM. We enabled cache for some use cases, but turned out it was over-optimized with stale data. We ended up turning off some caching points to ensure the data consistency.
...
*   Use built-in functions [https://docs.python.org/3/library/functions.html](https://docs.python.org/3/library/functions.html&sa=D&ust=1572350982004000). It is written in C, no one can beat it.
...
We also built [RBAC-based](https://en.wikipedia.org/wiki/Role-based_access_control&sa=D&ust=1572350982007000) permission system, along with Change Approval flow to make sure any prod change needs at least two people(and approver role has higher permission)
