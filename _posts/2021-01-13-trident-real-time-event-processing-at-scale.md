...
excerpt: "Find out where the messages and rewards come from, that arrive on your Grab app. Walk through scaling and processing optimizations that achieve tremendous throughput."
---
...
Trident is essentially Grab’s in-house real-time [if this, then that (IFTTT)](https://en.wikipedia.org/wiki/IFTTT) engine, which automates various types of business workflows. The nature of these workflows could either be to create awareness or to incentivize users to use other Grab services.
...
More specifically, a rule consists of one or more conditions combined by `AND/OR` Boolean operators. A condition consists of an operator with a left-hand side (LHS) and a right-hand side (RHS). The left-hand side is the name of a _variable_, and the right-hand side a value. A sample rule in JSON:
...
_First optimization - Lazy loading_

Lazy loading is a common performance optimization technique, which literally means _“don't do it until it’s necessary”_.
...
_Second optimization - Add weight_
