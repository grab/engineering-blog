...
In [Part I](https://engineering.grab.com/signals-market-place), we shared why Grab is investing in a data mesh (also known as *Signals Marketplace* in Grab) as our data culture evolves, and how data certification helps teams reliably reuse data across domains. But culture doesn’t change through principles alone; it changes when tools reshape people’s day-to-day behavior, making it critical for us to build effective platforms that embed these practices.

This follow-up focuses on the platforms that make certification work at Grab:
...
- **Data contracts and documentation**: Data contracts, classifications, and rich documentation live as structured metadata attached to each asset, not scattered across wikis and slide decks. Producers and consumers share the same source of truth when they ask, “What does this table guarantee?”  
...
The [open-source DataHub](https://datahub.com/)’s architecture is fundamentally event-driven and designed for high extensibility, moving away from the "passive" catalogs of the past toward a "living" metadata graph. DataHub models everything as an *Entity* (e.g., a Dataset), composed of multiple *Aspects* (atomic, versioned metadata like SchemaMetadata or Ownership), connected by *Relationships* (e.g., DownstreamOf).
...
<div class="post-image-section"><figure>
  <img src="/img/datamesh-part-two/figure-2.png" alt="" style="width:100%"><figcaption align="middle">Figure 2. Hubble’s system architecture at a glance.</figcaption>
  </figure>
</div>

This architecture matters for data mesh because metadata can move faster than organizational change. As domains evolve, tables move, or pipelines are refactored, Hubble updates continuously, and certification can keep up without manual re-audits.
...
Conceptually, assets move between four states: *Uncertified* (never met all criteria), *Certified*, *CertifiedPlus*, and *Revoked* (previously certified but now out of compliance). Certified means the asset itself meets all required criteria; CertifiedPlus is stricter, requiring both the asset and all of its upstreams to meet the criteria. The state diagram below captures how assets move between these states as metadata and upstream health change.

<div class="post-image-section"><figure>
  <img src="/img/datamesh-part-two/figure-3.png" alt="" style="width:80%"><figcaption align="middle">Figure 3. Certification state machine diagram.</figcaption>
  </figure>
</div>
...
* **Lineage and upstream trust**: For stronger levels like Certified Plus, upstream lineage must be present, and upstream assets must themselves be certified.  
...
- **Semantic validity (values & business rules).** Do the values themselves make sense? Column-level and X-Validation tests enforce constraints like uniqueness, ranges, patterns, cross-table reconciliations, and more advanced anomaly detection on row counts and null percentages.
...
<div class="post-image-section"><figure>
  <img src="/img/datamesh-part-two/figure-5.png" alt="" style="width:80%"><figcaption align="middle">Figure 5. Genchi’s system architecture at a glance.</figcaption>
  </figure>
</div>
...
This matters for data mesh because it turns “data as a product” from a slogan into something operational. Contracts give domain teams a clear, enforceable way to declare what they are willing to guarantee, and give consumers a concrete basis for trust and reuse across domains. Crucially, a contract is only useful if its promises are verifiable and enforceable. Schema expectations, quality checks, and SLAs are all linked to concrete tests and health endpoints, so downstream platforms can automatically detect breaches and prevent or triage breaking changes rather than treating the contract as static documentation.
